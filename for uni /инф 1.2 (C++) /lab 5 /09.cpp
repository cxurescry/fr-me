...
int main() {
    ...
    /*
    === ВЫВОД ===
    Корректность замены логических операций на побитовые в условных операторах:

    1. Логические операторы (&&, ||, !):
       - Используют short-circuit evaluation (сокращенное вычисление)
       - Для &&: если левая часть ложь, правая часть не вычисляется
       - Для ||: если левая часть истина, правая часть не вычисляется
       - Возвращают булево значение (true/false)
       - Оператор ! инвертирует булево значение

    2. Побитовые операторы (&, |, ~):
       - Всегда вычисляют оба операнда
       - Выполняют операции над каждым битом
       - Возвращают целочисленное значение
       - Оператор ~ инвертирует все биты числа

    3. ЗАМЕНА МОЖЕТ БЫТЬ НЕКОРРЕКТНОЙ:
       - Побитовые операторы не поддерживают short-circuit evaluation
       - Это может привести к нежелательным побочным эффектам
       - Оператор ~ работает по-другому: ~0 = -1 (все биты установлены), а не true
       - Для булевых значений в условиях можно использовать, но не рекомендуется
       - Для целочисленных значений в условиях: работает, но может давать неожиданные результаты
    */
